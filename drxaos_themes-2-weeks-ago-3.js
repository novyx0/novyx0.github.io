...
body .card-quality.camrip{background:rgba(239,68,68,0.18)!important;color:#f87171!important}
...
body .full-start__buttons .full-start__button:not(.drxaos-btn),
body .full-start-new__buttons .full-start__button:not(.drxaos-btn) {
    ...
    function applyDetailsStyles() {
        try {
            styleManager.setStyle('drxaos-details-style', `
                :root {
                    --drxaos-chip-min-height: clamp(38px, 5vw, 54px);
                    --drxaos-chip-radius: clamp(14px, 3vw, 20px);
                    --drxaos-chip-gap: clamp(0.45rem, 1vw, 0.75rem);
                    --drxaos-chip-padding-y: clamp(0.45rem, 0.9vw, 0.7rem);
                    --drxaos-chip-padding-x: clamp(0.9rem, 1.6vw, 1.35rem);
                }
                ...
                .full-start__details span {
                    ...
                    background: linear-gradient(135deg, rgba(var(--bg-rgb, 12, 14, 24), 0.85), rgba(var(--primary-rgb, 48, 65, 124), 0.32));
                    border: 1px solid rgba(var(--primary-rgb, 48, 65, 124), 0.36);
                    box-shadow: 0 12px 30px rgba(6, 9, 18, 0.45);
                    color: var(--text-main, #f1f5f9);
                    font-family: 'Netflix Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Helvetica Neue', Arial, sans-serif;
                    font-size: clamp(0.82rem, 0.5vw + 0.48rem, 0.98rem);
                    font-weight: 600;
                ...
                .full-start__details span:focus {
                    ...
                    box-shadow: 0 18px 38px rgba(6, 9, 18, 0.55);
        ...
        try {
            styleManager.setStyle('drxaos-reactions-style', `
                ...
                .full-start__reactions .reaction {
                    ...
                    min-height: var(--drxaos-chip-min-height);
                    border-radius: var(--drxaos-chip-radius);
                    background: linear-gradient(135deg, rgba(236, 72, 153, 0.18), rgba(var(--bg-rgb, 12, 14, 24), 0.86));
                    border: 1px solid rgba(236, 72, 153, 0.32);
                    box-shadow: 0 12px 30px rgba(6, 9, 18, 0.45);
                    color: var(--text-main, #fdf2f8);
                ...
                .full-start__reactions .reaction__count {
                    font-family: 'Netflix Sans', -apple-system, BlinkMacSystemFont, 'Segoe UI', 'Helvetica Neue', Arial, sans-serif;
                    font-size: clamp(0.85rem, 0.5vw + 0.5rem, 0.98rem);
                    font-weight: 600;
        ...
        try {
            styleManager.setStyle('drxaos-rate-line-style', `
                ...
                .full-start__rate-line .full-start__status {
                    ...
                    padding: var(--drxaos-chip-padding-y) var(--drxaos-chip-padding-x);
                    border-radius: var(--drxaos-chip-radius);
                    background: linear-gradient(135deg, rgba(var(--bg-rgb, 12, 14, 24), 0.88), rgba(59, 130, 246, 0.22));
                ...
                .full-start__rate-line .full-start__rate img {
                    ...
                    margin-right: clamp(0.25rem, 0.6vw, 0.4rem);
                }
                .full-start-new__rate-line .full-start__status {
                    text-transform: uppercase;
...
function addSettings() {
    ...
    Lampa.SettingsApi.addParam({
        ...
        if (value === 'custom') {
            var currentCustomKey = (Lampa.Storage.get('tmdb_api_key_custom', '') || Lampa.Storage.get('tmdb_api_key', '') || '').trim();
            inputOverlay.open({
                title: Lampa.Lang.translate('drxaos_overlay_tmdb_title'),
                message: Lampa.Lang.translate('drxaos_overlay_tmdb_hint'),
                ...
                if (result.action === 'clear') {
                    Lampa.Storage.set('tmdb_api_key_custom', '');
                    Lampa.Storage.set('tmdb_api_key', 'c87a543116135a4120443155bf680876');
                    if (Lampa.Noty) {
                        Lampa.Noty.show(Lampa.Lang.translate('tmdb_removed'));
                    }
                    applySeasonInfo();
                    scheduleDrxaosApplyAll();
                    return;
...
function syncActionButtons(context) {
    try {
        ...
        var sortedButtons = buttonInfoList.slice().sort(function(a, b) {
...
function initMovieQualitySystem(jacredUrl) {
    ...
    function addQualityToMiniCard(cardElement, cardData) {
        ...
        if (!qualityElement) {
            ...
            qualityElement.style.color = '#ffffff';
            qualityElement.style.background = 'rgba(0, 0, 0, var(--drxaos-surface-opacity))';
        }
        ...
        if (cacheQualityData && cacheQualityData.quality && cacheQualityData.quality !== 'undefined' && cacheQualityData.quality !== '') {
            ...
            if (cacheQualityData.isCamrip) {
                ...
                qualityElement.style.background = 'rgba(239, 68, 68, 0.18)';
